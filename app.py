...
# Makes cart_count available in every template (used by the shared navbar)
# without every single route having to calculate and pass it manually.
...
def item_list1():
    ...
    # NOTE: this used to be a separate "search only" page that rendered
    # item_list.html without passing `categories`, which crashed the
    # category dropdown on that template. The search box on the product
    # list now posts straight to /admin/item-list (see route 13 below),
    # so this route just forwards there, preserving the search term.
    return redirect(url_for('item_list', search=request.args.get('search', '')))
...
def user_product_details(product_id):
    ...
    return render_template("user/products_details.html", product=product)
...
# ==================================================================
# to create address page for user to enter address details before proceeding to payment
# =================================================================
